...
class OpenProtocolSerializer extends Transform {
  ...
  _transform(chunk, encoding, cb) {
    ...
    if (chunk.stationID === undefined || chunk.stationID === "  ") {
      ...
      chunk.stationID = pad(chunk.stationID, 2, 10); // Add base 10
    }
    ...
    if (chunk.spindleID === undefined || chunk.spindleID === "  ") {
      ...
      chunk.spindleID = pad(chunk.spindleID, 2, 10); // Add base 10
    }
    ...
    buf.write(pad(sizeMessage - 1, 4, 10), 0, 4, encodingOP);
    buf.write(pad(chunk.mid, 4, 10), 4, 4, encodingOP);
    buf.write(pad(chunk.revision, 3, 10), 8, encodingOP);
    buf.write(pad(chunk.stationID, 2, 10), 12, encodingOP);
    buf.write(pad(chunk.spindleID, 2, 10), 14, encodingOP);
    buf.write(pad(chunk.sequenceNumber, 2, 10), 16, encodingOP);
    buf.write(pad(chunk.messageParts, 1, 10), 18, encodingOP);
    buf.write(pad(chunk.messageNumber, 1, 10), 19, encodingOP);
